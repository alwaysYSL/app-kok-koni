...
void main() {
  group('Audit A-01 s/d A-09 & Section 5: Hardening Regression Suite', () {
    ...
    // -------------------------------------------------------------------------
    test('1. Skenario A-02: Race condition asinkron login-logout membatalkan commit token dan menghasilkan AuthSignedOut', () async {
    ...
    // -------------------------------------------------------------------------
    test('2. Skenario A-07: Single-flight bootstrap mencegah duplikasi restoreSession saat dipanggil secara simultan', () async {
      final restoreCompleter = Completer<AuthResult>();
      final repo = _ControlledAuthRepository(restoreCompleter: restoreCompleter);
      final storage = _InMemoryTokenStorage();
      SharedPreferences.setMockInitialValues({});
      final prefs = await SharedPreferences.getInstance();
      final skStore = RememberedSkStore(prefs);
    ...
    // -------------------------------------------------------------------------
    test('3. Skenario A-03: snapshotProvider melempar SessionRequiredException saat sessionScope null tanpa memanggil fetchDistrict', () async {
      final countingRepo = _CountingKokRepository();
      final container = ProviderContainer(
        overrides: [
          repositoryProvider.overrideWithValue(countingRepo),
        ],
      );
      addTearDown(container.dispose);
    ...
    // -------------------------------------------------------------------------
    testWidgets('4. Skenario A-01: Router guard mengunci rute internal saat AuthSigningIn dan AuthSigningOut', (tester) async {
    ...
    // -------------------------------------------------------------------------
    test('5. Skenario A-05: StorageException pada pembacaan token dialihkan ke AuthTemporarilyUnavailable dengan reason jujur', () async {
      final storage = _InMemoryTokenStorage()
        ..shouldThrow = true
    ...
    // -------------------------------------------------------------------------
    test('6. Skenario A-06: DemoAuthRepository menolak token acak/tidak valid tanpa fallback ke Garut Kota', () async {
      final storage = _InMemoryTokenStorage();
      ...
      expect(result.failure?.message, contains('Sesi Anda tidak valid atau telah kedaluwarsa.'));
    });

    // -------------------------------------------------------------------------
    // 7. Validasi validateAppConfiguration Fail-Closed di Lingkungan Produksi (Menutup A-04)
    // -------------------------------------------------------------------------
    test('7. Skenario A-04: validateAppConfiguration melempar StateError saat production memakai adapter/data demo', () {
    ...
    // -------------------------------------------------------------------------
    testWidgets('8. Skenario A-09: Rekapitulasi data menampilkan Tarogong Kidul secara dinamis pada modal sheet dan teks clipboard', (tester) async {
      SharedPreferences.setMockInitialValues({});
      final prefs = await SharedPreferences.getInstance();
      final storage = _InMemoryTokenStorage();
      final skStore = RememberedSkStore(prefs);
      ...
      expect(find.text('Rekapitulasi Data KOK Tarogong Kidul'), findsOneWidget);
      expect(
      ...
      expect(copiedClipboardText, contains('REKAPITULASI DATA KECAMATAN TAROGONG KIDUL'));
      expect(copiedClipboardText, contains('Status: Terdaftar pada Sistem KOK Kecamatan Tarogong Kidul'));
    ...
    // -------------------------------------------------------------------------
    test('9. Skenario Bagian 5: UserPrincipal memastikan permissions unmodifiable dan kesetaraan nilai menyeluruh', () {
