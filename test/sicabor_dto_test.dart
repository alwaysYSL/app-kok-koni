...
void main() {
  ...
  group('SicaborProfileResponse', () {
    test('parses profile envelope with scope, member, kontingen, summary, and data_notes', () {
      final json = {
        'success': true,
    ...
    test('parses profile envelope with null kontingen and missing data_notes', () {
      final json = {
        'success': true,
        'message': 'OK',
        'scope': {
    ...
    test('toJson and fromJson round-trip for SicaborProfileResponse', () {
      ...
      expect(fromJson.message, equals(response.message));
      expect(fromJson.scope.subdistrictId, equals(response.scope.subdistrictId));
      expect(fromJson.scope.subdistrictName, equals(response.scope.subdistrictName));
      ...
      expect(fromJson.data.member.username, equals(response.data.member.username));
      ...
      expect(fromJson.data.member.statusLabel, equals(response.data.member.statusLabel));
      expect(fromJson.data.kontingen?.id, equals(response.data.kontingen?.id));
      expect(fromJson.data.kontingen?.code, equals(response.data.kontingen?.code));
      expect(fromJson.data.kontingen?.name, equals(response.data.kontingen?.name));
      expect(fromJson.data.summary.totalCabor, equals(response.data.summary.totalCabor));
  ...
  group('SicaborErrorResponse', () {
    test('parses error envelope with error_code', () {
      ...
      expect(res.message, equals('Endpoint ini hanya dapat diakses oleh akun KOK.'));
